...
public interface AttemptRepository extends JpaRepository<Attempt, Long> {
    ...
    @Query("select a from Attempt a join fetch a.challenge where a.member.id = :memberId order by a.createdAt desc, a.id desc")
    ...
    Optional<Attempt> findTopByChallengeIdAndMemberIdOrderByCreatedAtDescIdDesc(Long challengeId, Long memberId);
    ...
    @Query("select a from Attempt a where a.challenge.id = :challengeId and a.member.id = :memberId order by a.createdAt asc, a.id asc")
    ...
    @Query("select a from Attempt a join fetch a.challenge where a.challenge.id = :challengeId and a.member.id = :memberId order by a.createdAt asc, a.id asc")
