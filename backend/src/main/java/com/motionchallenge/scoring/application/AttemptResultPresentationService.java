...
@Component
public class AttemptResultPresentationService {

    public AttemptResultPresentation buildPresentation(String attemptStatus, int score) {
        if (!AttemptStatus.COMPLETED.equals(attemptStatus)) {
            return new AttemptResultPresentation(
                    false,
                    "준비 단계 기록이 저장되었습니다.",
        ...
        if (score >= 80) {
            return new AttemptResultPresentation(
                    ...
                    "업로드 영상이 레퍼런스 흐름과 상당히 가깝게 인식되었습니다.");
        }
        ...
        return new AttemptResultPresentation(
                ...
                "업로드 영상과 레퍼런스 사이에 눈에 띄는 차이가 있습니다.");
    }
}
