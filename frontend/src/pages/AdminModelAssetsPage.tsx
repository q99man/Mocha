...
export function AdminModelAssetsPage() {
  ...
  async function handleAnalyzeReference(challengeId: number) {
    ...
    try {
      ...
      setAnalysisMessage(`레퍼런스 분석 완료: #${result.challengeId} / ${result.analyzerName ?? 'analyzer unknown'}`);
      await loadAdminData();
    } catch (submitError) {
    ...
    <div className="page admin-page">
      <section className="hero hero--detail">
        <div className="hero__content">
          <span className="hero__eyebrow">ADMIN / OPERATIONS HUB</span>
          <h2>운영 허브</h2>
          <p>모델 자산 업로드와 실제 레퍼런스 기반 챌린지 생성을 한 화면에서 관리합니다. seed profile 대신 real reference profile을 만들 때 이 페이지를 사용하면 됩니다.</p>
          <div className="signal-panel">
            <span className="signal-panel__label">CURRENT ACTIVE MODEL</span>
            <strong>{activeAsset ? activeAsset.originalFileName : 'NO ACTIVE MODEL'}</strong>
        ...
        <div className="hero__aside">
          <article className="panel panel--section">
            <div className="section-heading">
              ...
              <div>
                <h2>운영 현황</h2>
                <p>실 reference 기반으로 테스트 가능한 챌린지 수와 최근 모델 상태를 빠르게 확인합니다.</p>
              </div>
            </div>
            <div className="signal-grid">
              <div className="signal-grid__item">
                <span>ACTIVE MODEL</span>
                <strong>{activeAsset ? 'READY' : 'NONE'}</strong>
                <p>{activeAsset?.versionLabel ?? '모델 업로드 필요'}</p>
              </div>
              <div className="signal-grid__item">
                <span>READY CHALLENGES</span>
                <strong>{readyChallenges.length}</strong>
                <p>reference video + motion profile 준비 완료</p>
              </div>
              <div className="signal-grid__item">
                <span>LAST CREATED</span>
                <strong>{createdChallengeId ? `#${createdChallengeId}` : 'NONE'}</strong>
        ...
        <article className="panel panel--section">
          <div className="section-heading">
            ...
            <div>
              ...
              <p>실 reference video를 업로드해서 새 챌린지를 만들고, 이후 같은 화면에서 바로 분석까지 실행할 수 있습니다.</p>
            </div>
          </div>
      ...
      <section className="dashboard-grid admin-grid admin-grid--wide">
        <article className="panel panel--section">
          <div className="section-heading">
            ...
            <div>
              <h2>레퍼런스 분석 실행</h2>
              <p>reference video 업로드가 끝난 챌린지에서 실제 Motion Profile 생성을 실행합니다.</p>
            </div>
          </div>
            ...
            <div className="admin-asset-list">
              {challenges.map((challenge) => (
                <article key={challenge.id} className={`admin-asset-card${challenge.referenceMotionProfileReady ? ' admin-asset-card--active' : ''}`}>
                  ...
                  <p>reference video: {challenge.referenceVideoUploaded ? 'uploaded' : 'missing'}</p>
                  <p>analysis: {challenge.referenceAnalysisStatus}</p>
                  <p>profile: {challenge.referenceMotionProfileReady ? 'ready' : 'pending'}</p>
                  <div className="inline-actions">
                    <button
              ...
              {assets.map((asset) => (
                <article key={asset.id} className={`admin-asset-card${asset.active ? ' admin-asset-card--active' : ''}`}>
                  <div className="admin-asset-card__header">
                    ...
                    <span className="pill">{asset.active ? 'ACTIVE' : 'ARCHIVE'}</span>
                  </div>
